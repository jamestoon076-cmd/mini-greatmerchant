...
def update_game_time(player, settings, market_data, initial_stocks):
    ...
    months_passed = min(int(elapsed / 180), 12)
    events = []
...
def auto_save(doc):
    if time.time() - st.session_state.last_auto_save > 300:
                    ...
                    if items:
                        ...
                        for item_name in items:
                            try:
                                            ...
                                            if qty_int > 0:
                                                ...
                                                if actual_qty > 0:
                                                    ...
                                                    if bought > 0:
                                                        ...
                                                        avg_price = spent // bought if bought > 0 else 0
                                                        st.markdown(
                                                            f"<div class='trade-complete'>✅ 총 {bought}개 매수 완료! "
                                                            f"(총 {spent:,}냥 | 평균가: {avg_price:,}냥)</div>", 
                                                            unsafe_allow_html=True
                                                        )
                                                    else:
                                ...
                                st.error(f"⚠️ 아이템 {item_name} 처리 중 오류: {e}")
                    else:
            ...
            if player['inv']:
                ...
                for item, qty in player['inv'].items():
                    if qty > 0 and item in items_info:
                        total_value += items_info[item]['base'] * qty
                        st.write(f"• {item}: {qty}개")
                st.info(f"💰 예상 가치: {total_value:,}냥 (기준가)")
            else:
