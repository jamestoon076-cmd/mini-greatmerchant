            ...
            try:
                ...
                while total_bought < qty:
                    ...
                    if time.time() - last_log_time > 0.5:
                        진행률 = (total_bought / qty) * 100 if qty > 0 else 0
                        with progress_placeholder.container():
                            st.markdown("<div class='trade-progress'>", unsafe_allow_html=True)
                            st.markdown(f"<div class='trade-line'>➤ 진행률: {진행률:.1f}% ({total_bought}/{qty})</div>", unsafe_allow_html=True)
                            st.markdown(f"<div class='trade-line'>➤ 현재 체결가: {target['price']}냥</div>", unsafe_allow_html=True)
                            st.markdown(f"<div class='trade-line'>➤ 남은 무게: {tw - cw}근</div>", unsafe_allow_html=True)
                            st.markdown(f"<div class='trade-line'>➤ 남은 돈: {player['money']:,}냥</div>", unsafe_allow_html=True)
                ...
                if total_bought > 0:
                    ...
                    st.session_state.last_trade_result = f"✅ {item_name} 총 {total_bought}개 매수 완료! (총 {total_spent:,}냥 | 평균가: {avg_price}냥)"
        ...
        def process_sell(player, items_info, market_data, pos, item_name, qty, progress_placeholder, log_key):
            ...
            try:
                ...
                while total_sold < qty:
                    ...
                    if time.time() - last_log_time > 0.5:
                        진행률 = (total_sold / qty) * 100 if qty > 0 else 0
                        with progress_placeholder.container():
                            st.markdown("<div class='trade-progress'>", unsafe_allow_html=True)
                            st.markdown(f"<div class='trade-line'>➤ 진행률: {진행률:.1f}% ({total_sold}/{qty})</div>", unsafe_allow_html=True)
                            st.markdown(f"<div class='trade-line'>➤ 현재 체결가: {current_price}냥</div>", unsafe_allow_html=True)
                            st.markdown(f"<div class='trade-line'>➤ 누적 수익: {total_earned:,}냥</div>", unsafe_allow_html=True)
                            st.markdown("</div>", unsafe_allow_html=True)
                        last_log_time = time.time()
                ...
                if total_sold > 0:
                    avg_price = total_earned // total_sold
                    st.session_state.last_trade_result = f"✅ {item_name} 총 {total_sold}개 매도 완료! (수익: {total_earned:,}냥 | 평균가: {avg_price}냥)"
        ...
        if doc:
            if not st.session_state.game_started:
                ...
                if slots:
                    ...
                    if st.button("🎮 게임 시작", use_container_width=True):
                        ...
                        if selected:
                            ...
                            st.session_state.last_time_update = time.time()
                            st.session_state.trade_logs = {}
                            st.session_state.tab_key = 0  # 탭 키 초기화
                ...
                # 거래 중이 아닐 때만 시간 업데이트
                if not st.session_state.get('is_trading', False):
                    ...
                    if items:
                        ...
                        for item_name in items:
                            ...
                            with st.container():
                                ...
                                # 최대 버튼
                                if col_d.button("🔝 최대", key=f"max_{item_name}", use_container_width=True):
                                    max_possible = calculate_max_purchase(player, items_info, market_data, player['pos'], item_name, d['price'])
                                    st.session_state.last_qty[f"{player['pos']}_{item_name}"] = str(max_possible)
                                    st.rerun()
