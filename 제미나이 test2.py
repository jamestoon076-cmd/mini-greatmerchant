                ...
                slots.append(
                    {
                        ...
                        "inv": (
                            json.loads(r.get("inventory", "{}"))
                            if r.get("inventory")
                            else {}
                        ),
                        "mercs": (
                            json.loads(r.get("mercs", "[]")) if r.get("mercs") else []
                        ),
    ...
    if weeks_passed > 0:
        for _ in range(weeks_passed):
            ...
            if player["week"] > 4:
                ...
                events.append(
                    ("month", "📅 새 달이 밝아 모든 마을의 재고가 초기화되었습니다!")
                )
    ...
    if months_passed > 0:
        ...
        if old_month != player["month"] or old_year != player["year"]:
            events.append(
                (
                    "month",
                    f"🌙 {player['year']}년 {player['month']}월이 시작되었습니다!",
                )
            )
                ...
                if v_name in initial_stocks:
                    for item_name in market_data[v_name]:
                        if item_name in initial_stocks[v_name]:
                            ...
                            market_data[v_name][item_name]["stock"] = initial_stocks[
                                v_name
                            ][item_name]
        ...
        events.append(
            ("week", f"🌟 {player['year']}년 {player['month']}월 {player['week']}주차")
        )
        ...
        if random.random() < event_probability:
            ...
            if cities:
                ...
                if items_in_city:
                    ...
                    vol_amount = random.randint(10, 30) + int(
                        inventoryResponsivePrice / 1000
                    )

                    if vol_direction == "상승":
                        market_data[random_city][vol_item]["price"] = int(
                            market_data[random_city][vol_item]["price"]
                            * (1 + vol_amount / 100)
                        )
                        events.append(
                            (
                                "volatility",
                                f"📈 {random_city}의 {vol_item} 가격 {vol_amount}% 급등!",
                            )
                        )
                    else:
                        market_data[random_city][vol_item]["price"] = int(
                        ...
                        events.append(
                            (
                                "volatility",
                                f"📉 {random_city}의 {vol_item} 가격 {vol_amount}% 급락!",
                            )
                        )
...
def calculate_max_purchase(
    player, items_info, market_data, pos, item_name, target_price
):
...
def process_buy(
    player, items_info, market_data, pos, item_name, qty, progress_placeholder, log_key
):
    total_bought = 0
    total_spent = 0
    ...
    while total_bought < qty:
        ...
        can_load = (
            ...
            if items_info[item_name]["w"] > 0
            else 999999
        )

        # 3. 이번 턴에 체결할 양 (남은양, 100개단위, 재고, 돈, 무게 중 최소값)
        current_batch = min(
            batch_size, qty - total_bought, target["stock"], can_pay, can_load
        )
        ...
        with progress_placeholder.container():
            for log in st.session_state.trade_logs[log_key][-5:]:
                st.markdown(
                    f"<div class='trade-line'>{log}</div>", unsafe_allow_html=True
                )
    ...
    if total_bought > 0:
        st.session_state.last_trade_result = (
            f"✅ {item_name} 총 {total_bought}개 구매 완료! (총 {total_spent:,}냥)"
        )
...
def process_sell(
    ...
    while total_sold < qty:
        ...
        current_batch = min(
            batch_size, qty - total_sold, player["inv"].get(item_name, 0)
        )
    ...
    if total_sold > 0:
        st.session_state.last_trade_result = (
            f"✅ {item_name} 총 {total_sold}개 판매 완료! (총 {total_earned:,}냥)"
        )
...
def save_player_data(doc, player, stats, device_id):
    ...
    if doc:
        if not st.session_state.game_started:
            ...
            settings, items_info, merc_data, villages, initial_stocks, slots = (
                load_game_data()
            )
            ...
            if slots:
                ...
                if st.button("🎮 게임 시작", use_container_width=True):
                    selected = next(
                        (s for s in slots if s["slot"] == slot_choice), None
                    )
                    if selected:
...
if doc:
    if not st.session_state.game_started:
                            ...
                            if len(player["mercs"]) >= max_mercs:
                                ...
                                if st.button(
                                    ...
                                    if player["money"] >= data["price"]:
                                        ...
                                        cw, tw = get_weight(
                                            player, items_info, merc_data
                                        )
                                        weight_placeholder.metric(
                                            "⚖️ 무게", f"{cw}/{tw}근"
                                        )
                                        money_placeholder.metric(
                                            "💰 소지금", f"{player['money']:,}냥"
                                        )
                                        st.success(
                                            f"✅ {name} 고용 완료! (총 {len(player['mercs'])}/{max_mercs}명)"
                                        )
                                        st.rerun()
                                    else:
                                        st.error("❌ 잔액 부족")
                    ...
                    for item_name in items:
                        ...
                        with st.container():
                            ...
                            if result_key in st.session_state:
                                st.markdown(
                                    f"<div class='trade-complete'>{st.session_state[result_key]}</div>",
                                    unsafe_allow_html=True,
                                )
                            ...
                            price_ph = col1.empty()
                            price_ph.markdown(
                                f"<span class='{price_class}'>{d['price']:,}냥</span>",
                                unsafe_allow_html=True,
                            )
                            ...
                            default_qty = st.session_state.last_qty.get(
                                f"{player['pos']}_{item_name}", "1"
                            )
                            qty = col_a.text_input(
                                "수량",
                                value=default_qty,
                                key=f"qty_{player['pos']}_{item_name}",
                            ...
                            for key in list(st.session_state.trade_logs.keys()):
                                if key.startswith(f"{player['pos']}_{item_name}"):
                                    with progress_ph.container():
                                        ...
                                        for log in st.session_state.trade_logs[key][
                                            -10:
                                        ]:
                                            st.markdown(
                                                f"<div class='trade-line'>{log}</div>",
                                                unsafe_allow_html=True,
                                            )
                            ...
                            if col_b.button(
                                ...
                                try:
                                    ...
                                    if qty_int > 0:
                                        log_key = (
                                            f"{player['pos']}_{item_name}_{time.time()}"
                                        )
                                        ...
                                        if bought > 0:
                                            ...
                                            st.session_state.last_qty[
                                                f"{player['pos']}_{item_name}"
                                            ] = "1"
                                            st.rerun()
                                        else:
                                            st.error(
                                                "❌ 구매 가능한 수량이 없거나 돈/무게가 부족합니다."
                                            )
                                    else:
            ...
            if player["mercs"]:
                ...
                for merc, count in merc_count.items():
                    if merc in merc_data:
                        ...
                        # 해고 버튼
                        if col4.button(
                            f"❌ 해고", key=f"fire_{merc}", use_container_width=True
                        ):
                            # 해당 용병 1명 제거
        ...
        with tab4:
            ...
            with col1:
                st.metric(
                    "💰 총 구매액", f"{st.session_state.stats['total_spent']:,}냥"
                )
                st.metric(
                    "📦 총 구매량", f"{st.session_state.stats['total_bought']:,}개"
                )
                st.metric(
                    "🔄 총 거래 횟수", f"{st.session_state.stats['trade_count']}회"
                )

            with col2:
                st.metric(
                    "💵 총 판매액", f"{st.session_state.stats['total_earned']:,}냥"
                )
                st.metric("📦 총 판매량", f"{st.session_state.stats['total_sold']:,}개")

                # 순이익 계산
                net_profit = (
                    st.session_state.stats["total_earned"]
                    - st.session_state.stats["total_spent"]
                )
        ...
        with tab5:
            ...
            if player["pos"] in villages:
                ...
                for t in towns:
                    if t != player["pos"]:
                        dist = math.sqrt(
                            (curr_v["x"] - villages[t]["x"]) ** 2
                            + (curr_v["y"] - villages[t]["y"]) ** 2
                        )
                ...
                if move_options:
                    selected_text = st.selectbox(
                        "목적지 선택", move_options, key="move_selectbox"
                    )
            ...
            if st.button("💾 저장", use_container_width=True):
                if save_player_data(
                    doc, player, st.session_state.stats, st.session_state.device_id
                ):
                    st.success("✅ 저장 완료!")
